...
#   "pure_int8": 1 float16 scale + BLOCK_SIZE int8 values per block, no outlier tracking (PureInt8Linear)
#   "col_int8" : outlier columns kept as float16, remaining columns block-quantised to int8 (ColOutlierLinear)
QUANT_MODE       = "col_int8"
...
# ── Column-outlier INT8 quantisation (col_int8 mode) ─────────────────────────
def _col_int8_matmul(
    ...
    """
    col_int8 mode — fused dequant + matmul for non-outlier columns.

    Correct dequant path: int8 → /127 → inverse compand → × absmax scale.
    Accumulates in float32 to prevent fp16 overflow.

    qweight : [N, K_pad]               int8
    scales  : [N, K_pad // BLOCK_SIZE] float16
    x       : [..., K_normal]          any float dtype
    """
    N, K_pad   = qweight.shape
    num_blocks = K_pad // BLOCK_SIZE
...
class ColOutlierLinear(nn.Module):
    """
    col_int8 mode — drop-in replacement for nn.Linear.

    Columns whose per-row absmax exceeds  mean + COL_OUTLIER_SIGMA × std  are
    stored verbatim as float16.  All other columns are block-quantised to int8
    using the same scheme as PureInt8Linear (with optional companding).
    ...
    def __init__(self, linear: nn.Linear) -> None:
        ...
        # Per-column absmax; outlier = significantly above average
        col_absmax   = weight.abs().amax(dim=0)                              # [K]
        threshold    = col_absmax.mean() + COL_OUTLIER_SIGMA * col_absmax.std().clamp(min=1e-8)
        outlier_mask = col_absmax > threshold                                # [K] bool

        normal_idx  = (~outlier_mask).nonzero(as_tuple=False).squeeze(1)    # [num_normal]  int64
        outlier_idx =   outlier_mask.nonzero(as_tuple=False).squeeze(1)     # [num_outlier] int64

        self.register_buffer("normal_col_idx",  normal_idx)
        self.register_buffer("outlier_col_idx", outlier_idx)

        # Outlier columns: store verbatim as fp16
        if outlier_idx.numel() > 0:
            self.register_buffer("W_fp16", linear.weight.data[:, outlier_idx].to(torch.float16))
        else:
            self.W_fp16 = None

        # Normal columns: block-wise INT8 (reuses quantize_int8_block + companding)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # INT8 block GEMM on non-outlier columns
        if self.W_int8 is not None:
            ...
            if USE_FUSED_MATMUL:
                ...
                W   = dequantize_int8_block(self.W_int8, self.W_int8_scales,
                                             (self.out_features, self._normal_K))
                out = F.linear(x_normal, W)

        # FP16 GEMM on outlier columns
...
def quantize_selected_layers(model: nn.Module, layers_file: str) -> nn.Module:
    ...
    for full_name in names:
        ...
        if QUANT_MODE == "mixed":
            cls = MixedQuantLinear
        elif QUANT_MODE == "col_int8":
            cls = ColOutlierLinear
        else:
            cls = PureInt8Linear
...
def count_layers(model: nn.Module) -> tuple[int, int]:
    ...
    for m in model.modules():
        if isinstance(m, (nn.Linear, MixedQuantLinear, PureInt8Linear, ColOutlierLinear)):
            total += 1
            if isinstance(m, (MixedQuantLinear, PureInt8Linear, ColOutlierLinear)):
                quant += 1
    return quant, total
